...
def generate_simulator_function(tseg=10.0, dt_hires=1e-5, dt=0.005, 
    ...
    def simulation(param):
        ...
        if summary_type == "psd":
            ...
            aps = AveragedPowerspectrum(lc1_dt+lc2_dt, segment_size, norm="frac", silent=True)
...
def main():
    ...
    simulation_kwargs = {"tseg":10.0, "dt_hires":1e-5, "dt":0.0005, "deadtime":0.0025, 
                         "summary_type":"avglogbin", "segment_size":1.0, "f":0.01}
    ...
    for i in range(num_iter):
        ...
        np.savetxt("sim_hf_avgpsd_logbin_theta%i.dat"%i, np.array(theta))
        np.savetxt("sim_hf_avgpsd_logbin_x%i.dat"%i, np.array(x))
