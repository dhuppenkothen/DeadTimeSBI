...
def main():
    ...
    simulation_kwargs = {"tseg":10.0, "dt_hires":1e-5, "dt":0.5/750.0, "deadtime":0.0025, "summary_type":"logbin", "f":f}
    ...
    for i in range(num_iter):
        ...
        theta, x = simulate_for_sbi(simulator_wrapper, prior, num_simulations=5000)
        np.savetxt("sim_hf_psd_embed_theta%i.dat"%i, np.array(theta))
        np.savetxt("sim_hf_psd_embed_x%i.dat"%i, np.array(x))
